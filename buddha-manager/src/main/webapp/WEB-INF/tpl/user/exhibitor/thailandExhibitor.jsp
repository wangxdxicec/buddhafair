...
<div id="tabs" class="easyui-tabs" data-options="fit:true,border:false,plain:true">
	<div title="展商列表" style="padding:5px">
		<table id="exhibitors" data-options="url:'${base}/user/queryExhibitorsByPage?type=2',
            								 loadMsg: '数据加载中......',
...
<div id="importExhibitorsDlg" data-options="iconCls:'icon-add',modal:true">
	<form id="importExhibitorsForm" name="importExhibitorsForm">
	   	<table style="width: 320px;margin: 20px auto">
			<tr>
				<td colspan="2" style="width: 90px;text-align: left"><a href="${base}/resource/thailandExhibitorTemplate.xlsx">下载导入模版</a><br /></td>
			</tr>
			<tr>
...
<div id="toolbar">
	...
	<div id="import" style="width:100px;">
		<div id="importExhibitorTemplate" iconCls="icon-undo"><a href="${base}/resource/thailandExhibitorTemplate.xlsx">下载导入模版</a></div>
...
//	        				    console.log(a);
	...
	$('#exportAllTransactions').click(function(){
		...
		var node2 = "<input type='hidden' name='type' value='2'/>";
					...
					$.messager.confirm('确认导入','你确定要导入选中的资料?',function(r){
						if (r){
							$.ajax({
								...
								type: "post",
								dataType: "json",
								data: {"tids": willImportCheckedItems, "type":"2"},
								traditional: true,
								success: function (data) {
									if (data.resultCode == 0) {
										$.messager.show({
					...
					$.messager.confirm('确认删除','你确定要删除选中的资料?',function(r){
						if (r){
							$.ajax({
								...
								type: "post",
								dataType: "json",
								data: {"tids": isExistCheckedItems, "type":"2"},
								traditional: true,
								success: function (data) {
									if (data.resultCode == 0) {
										$.messager.show({
