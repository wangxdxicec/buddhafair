...
<div id="tabs" class="easyui-tabs" data-options="fit:true,border:false,plain:true">
	<div title="境外法师列表" style="padding:5px">
		<table id="customers" data-options="url:'${base}/user/queryForeignRabbiCustomersByPage',
         						   loadMsg: '数据加载中......',
...
<div id="customerbar">
	...
	<div id="email" style="width:180px;">
		<div id="emailAllCustomers" iconCls="icon-redo">群发所有境外法师邮件</div>
		<div id="emailSelectedCustomers" iconCls="icon-redo">群发所选境外法师邮件</div>
	</div>
	<div style="display:inline-block;">
		<div class="easyui-menubutton" menu="#export" iconCls="icon-redo">导出</div>
	</div>
	<div id="export" style="width:180px;">
		<div id="exportAllCustomers" iconCls="icon-redo">所有境外法师信息到Excel</div>
		<div id="exportSelectedCustomers" iconCls="icon-redo">所选境外法师信息到Excel</div>
	</div>
</div>
...
<script>
	...
	$('#emailAllCustomers').click(function(){
		$.messager.confirm('确认删除','你确定要群发所有境外法师邮件吗?',function(r){
			if (r){
				$.ajax({
	...
	$('#emailSelectedCustomers').click(function(){
		if(checkedItems.length > 0){
			$.messager.confirm('确认删除','你确定要群发所选境外法师邮件吗?',function(r){
				if (r){
					$.ajax({
						url: "${base}/user/emailAllForeignCustomers",
						type: "post",
			...
			$.messager.alert('提示', '请至少选择一项境外法师再操作');
		}
	});
	//导出所有境外法师信息到Excel
...
//     	alert(cidParm1.innerHTML);
		if(checkedItems.length > 0){
			...
			cidParm2.innerHTML += node1;
			document.getElementById("exportForeignCustomersToExcel").submit();
			$.messager.alert('提示', '导出所选境外法师成功');
		}else{
			$.messager.alert('提示', '请至少选择一项境外法师再导出');
		}
	});
	//----------------------------------------------------工具栏函数结束--------------------------------------------------------//
	//----------------------------------------------------自定义函数开始--------------------------------------------------------//
